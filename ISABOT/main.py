...
import courses
import flow_routing
import departments_routing
import qfaq_routing
from FLOW import english_flow, learn_flow, hr_flow, bs_flow, ai_flow
from DEPARTMENTS import english_dep, learn_dep, hr_dep, bs_dep, ai_dep, scholar_dep, cert_dep, reg_dep, faq_dep
from QFAQ import qfaq1, qfaq2, qfaq3, qfaq4, qfaq5, qfaq6, back
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(start.start, pattern="^(start)$")
    )
    application.add_handler(
    ...
    application.add_handler(
        CallbackQueryHandler(start.button)
    )
    ...
    async def handle_registration_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
       ...
       application.add_handler(CallbackQueryHandler(handle_registration_button, pattern="^register$"))
